...
class Evostrat:
    def evostrat(stocks):
        ...
        with open("weight_dict.txt", "r") as weight_dict:
            ...
            datafiles = []
            for s in stocks:
                datafiles.append("datasets/" + s + ".csv")
            ...
            for file in datafiles:
                with open(file, "r") as f:
                    ...
                    SMA = TI.SMA(myTI, close_prices[1: number_of_samples])
                    EMA = TI.EMA(myTI, close_prices[1: number_of_samples])
                    GROW = TI.GROW(myTI, close_prices[1: number_of_samples])
                    MACD = TI.MACD(myTI, close_prices[1: number_of_samples])
                    STOCH = TI.STOCH(myTI, low_prices[1: number_of_samples], high_prices[0: number_of_samples],close_prices[0: number_of_samples])
                    RSI = TI.RSI(myTI, close_prices[1: number_of_samples])
                    AROONUP = TI.AROONUP(myTI, close_prices[1: number_of_samples])
                    AROONDOWN = TI.AROONDOWN(myTI, close_prices[1: number_of_samples])
                    ...
                    grow_dict.append(expected_grow / close_prices[1])
        return grow_dict
